...
uses Windows,Forms,SysUtils,dialogs,Controls,
     EtvView,EtvForms,EtvPrint,EtvDB,EtvDBFun;
...
Procedure AddSum(aTableIndex:byte; aFieldName: string);
var i,k,index:smallint;
    SumItem:TSumItem;
    aField:TField;
begin
  ...
  for i:=0 to Sums.Count-1 do
    if (TSumItem(Sums[i]).TableIndex=aTableIndex) and (TSumItem(Sums[i]).FieldName=aFieldName) then begin
      Index:=i;
      Break;
    end;
  if Index=-1 then begin
    SumItem:=TSumItem.Create;
    SumItem.FieldName:=aFieldName;
    SumItem.TableIndex:=aTableIndex;
    SumItem.Value:=0;
    Sums.Add(SumItem);
    Index:=Sums.Count-1;
  end;
  { Имеем поле без таблицы, т.к. называемый Additional }
  if aTableIndex=0 then begin
    k:=NumAdditional(aFieldName);
    if k>=0 then begin
      TSumItem(Sums[Index]).Value:=TSumItem(Sums[Index]).Value+
        StrToFloat(TaItem(Additional.Items[k]).Value);
    end;
  end else begin
    aField:=TTbItem(DataSets.Items[TbIndex]).DataSet.FindField(aFieldName);
    if aField.Value<>Null then
      TSumItem(Sums[Index]).Value:=TSumItem(Sums[Index]).Value+aField.Value;
  end;
end;

Procedure WrtSum(aTableIndex:byte; aFieldName: string);
var i,k:smallint;
    s0:string;
    aField: TField;
begin
  for i:=0 to Sums.Count-1 do
    if (TSumItem(Sums[i]).TableIndex=aTableIndex) and (TSumItem(Sums[i]).FieldName=aFieldName) then begin
      ...
      if aTableIndex=0 then begin
        k:=NumAdditional(aFieldName);
        if k>=0 then s0:=TaItem(Additional.Items[k]).Value
      end else begin
        aField:=TTbItem(DataSets.Items[TbIndex]).DataSet.FindField(aFieldName);
      ...
      if Assigned(fBeforeSumFieldPrint) then
        fBeforeSumFieldPrint(Self,TSumItem(Sums[i]).Value,s0,aTableIndex,aFieldName);

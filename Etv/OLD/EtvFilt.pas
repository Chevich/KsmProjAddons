...
uses TypInfo,SysUtils,Windows,Forms,
     ...
     EtvTable,
     {$ENDIF}
     EtvPas,EtvConst,Filter,EtvForms,EtvOther,EtvTemp,
     DBCommon;
...
begin
  if ((fOrigDataSet<>DataSet) and (not fSetProcess)) then begin
    RestoreDataSet(true);
    CaptureDataSet;
    fUsed:=fuNone;
    LoadFromBase;
...
{Lev 09.02.2000}
      ...
      if lNeedQuery then begin
        ...
        if lWhere<>'' then begin
          try
            if fOldFiltered and (fOldFilter<>'') and (fOldFilter<>'1=2') then
            ...
            if Assigned(fQExtracts) then
              CreateSQLforQuery(fOrigDataSet,fQExtracts,GetTableName,'EFilter',lWhere,
                SortingFromDataSet(DataSource.DataSet),fTableNameKind)
            else begin
              ...
              if Not Assigned(fQExtracts) then
                QExtracts:=CopyDataSetToQuery(Self,fOrigDataSet,GetTableName,
                  'EFilter',lWhere,SortingFromDataset(DataSource.DataSet),fTableNameKind);
            end;

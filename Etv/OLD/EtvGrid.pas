unit EtvGrid;   (* Igo Val *)

interface

Uses Windows, Classes, Controls, Messages, Graphics, Grids,
     StdCtrls, DB, DBGrids, EtvLook, DBCtrls, EtvContr, EtvMem;
...
Uses TypInfo,SysUtils,Forms,Menus,Dialogs,
     EtvDBFun,EtvConst,EtvDB,EtvBor,EtvPrint,EtvPas,etvForms,etvOther,EtvPopup,
     DiDual,DiPrint;
...
begin
  ...
  FLookupCombo.ListLink.DataSet.
    Locate(TDBLookupControlBorland(FLookupCombo).FKeyFieldName,
          FLookupCombo.KeyValue, []);
  BWidth:=0;
  if BorderStyle=bsSingle then
    if Ctl3D then BWidth:=2 else BWidth:=1;
...
procedure TEtvDBGrid.MouseDown(Button: TMouseButton; Shift: TShiftState;
                               X, Y: Integer);
begin
...
var
  ...
  i,j:integer;
  FieldsList:TList;
procedure DrawLine(x1,y1,x2,y2:longint;aWidth:integer);
var
  OldPen: TPen;
begin
  OldPen := TPen.Create;
...
begin
  FIntoField:=false;
  if Assigned(DataLink) and (ACol >= IndicatorOffset) and
     (not(gdFixed in AState)) and
     Assigned(Columns[ACol-IndicatorOffset].Field) and
     (Columns[ACol-IndicatorOffset].Field is TEtvLookField) then begin
    Canvas.Font:=Font;
    TextWidth :=Canvas.TextWidth('0');
    ...
    for i:=0 to FieldsCount-1 do begin
      ...
      if (i=FieldsCount-1) and (PartRect.Right<ARect.Right) then
        PartRect.Right:=ARect.Right;
      DBGridDrawCell(ACol,ARow,PartRect, AState);
      DrawLine(PartRect.Right, PartRect.Top, PartRect.Right, PartRect.Bottom,1);
      if (FieldsList[i]=Field.LookupField[Field.ListFieldIndex]) and
         HighlightCell(ACol, ARow, '', AState) then
        DrawLine(PartRect.Left+1, PartRect.Bottom-3, PartRect.Left+13, PartRect.Bottom-3,1);
    ...
    Field.VFieldIndex:=-1;
  end else DBGridDrawCell(ACol,ARow,ARect,AState);
end;

...
uses
  ...
  StdCtrls, Buttons, ExtCtrls, Mask, DB, DBCtrls,
  EtvLook,EtvMem,EtvFilt;
...
implementation
uses TypInfo,Dialogs,EtvTemp,
     EtvConst,EtvDB,EtvPas,EtvContr,EtvForms;

{$R *.DFM}

const SizeAllOrAny=70;
...
begin
  with TViewItem(ListConditions.items[Index]) do begin
    ...
    with DataSetName do begin
      ...
      Items.Add(ObjectStrProp(EFilters.DataSet,'Caption'));
      ItemIndex:=0;
      for i:=0 to EFilters.SubDataSets.Count-1 do begin
        s:=ObjectStrProp(EFilters.SubDataSets[i],'Caption');
        if s='' then s:=EFilters.SubDataSets.TableName[i];
        Items.Add(s);
      end;
    end;
  end;
...
begin
  with TViewItem(ListConditions.items[Index]).Operation do begin
    ...
    Items.Add('=');
    Items.Add('<>');
    if Not (Assigned(Field) and (Field is TEtvLookField) and
           (foOnlyEqualInFilter in TEtvLookField(Field).options)) then begin
      Items.Add('<');
      Items.Add('>');
      Items.Add('<=');
      Items.Add('>=');
...
begin
  with TViewItem(ListConditions.items[Index]) do begin
    ...
    if Not(Value is TEtvDBLookupCombo) then
      TWinControlSelf(Value).OnKeyDown:=EditKeyDown;
...
var Control:TControl;
    ...
    PropInfo:PPropInfo;
begin
  Control:=TViewItem(ListConditions[index]).Value;
  Value:=TConditionItem(TFilterItem(EFilters.
        Filters[EFilters.CurFilter]).Conditions[Index]).Value;

  Scroll.InsertControl(Control);
  TWinControl(Control).TabOrder:=
    TViewItem(ListConditions.items[Index]).Operation.TabOrder+1;
  if Control is TComboBox then
    TComboBox(Control).Items.Assign(TEtvListField(FieldByIndex(Index)).Values);

  if Control is TDBLookupComboBox then begin
    TDBLookupComboBox(Control).KeyValue:=Value
  end else if Control is TCustomComboBox then
    if Value<>null then TCustomComboBox(Control).ItemIndex:=Value
    else TCustomComboBox(Control).ItemIndex:=-1
  else begin
    Exist:=false;
    if Value<>null then begin
      PropInfo:=GetPropInfo(Control.ClassInfo, 'Date');
      if PropInfo <> nil then begin
        SetFloatProp(Control,PropInfo,Value);
        Exist:=true;
      end;
    end;
    if Not Exist then begin
      ...
      if PropInfo <> nil then
        if Value<>null then SetStrProp(Control,PropInfo,Value)
        else SetStrProp(Control,PropInfo,'');
    end;
  end;
end;
  ...
  if Assigned(EFilters) and Assigned(EFilters.DataSet) then begin
    ...
    for i:=0 to EFilters.SubDataSets.Count do begin
      ...
      if EFilters.Enhanced then begin
        ...
        {$ENDIF}
        if i=0 then
          LabelTotal.Caption:=ObjectStrProp(EFilters.DataSet,'Caption')
        else begin
          LabelTotal.Caption:=ObjectStrProp(EFilters.SubDataSets[i-1],'Caption');
          if LabelTotal.Caption='' then
            LabelTotal.Caption:=EFilters.SubDataSets.TableName[i-1];
        end;
        ...
        if i>0 then begin
          ...
          AllOrAny.OnKeyDown:=EditKeyDown;
          {$IFDEF Delphi4}
      ...
      EditTotal.OnKeyDown:=EditKeyDown;
...
procedure TFormFilter.EditKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
begin
  if Shift=[ssCtrl] then
    case Key of
      VK_Add: begin SBAddCond.Click; key:=0; end;
      VK_SUBTRACT: begin SBDeleteCond.Click; key:=0; end;
    end;
end;

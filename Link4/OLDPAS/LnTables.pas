...
Uses Classes, Controls, Forms,  DBTables, BDE, DB, EtvTable, EtvDBase, EtvFilt;
...
Uses Windows, SysUtils, StdCtrls, DBConsts, Dialogs, IniFiles,
     LnPasw, XMisc, XCracks, XDBMisc, VgUtils;
...
Function GetCloneDataSet(aDataSet:TDataSet; aFieldList:TList):TDBDataSet;
var NotOwnerFlag: boolean;
begin
  try
    Result:=TDBDataSet(CreateClone(aDataSet));
    ...
    if aDataSet is TQuery then begin
      ...
      (TDBDataSet(aDataSet).FieldCount<>0) then
...
begin
  with Self do begin
    for i:=0 to FieldCount-1 do begin
      AField:=Fields[i];
      if AField.Lookup then
        if Assigned(AField.LookupDataSet) then begin
...
Procedure TLnTable.InternalInitRecord(Buffer: PChar);

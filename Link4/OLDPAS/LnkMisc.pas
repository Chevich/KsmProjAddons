...
{ TFindLink }
  ...
  private
    ...
    procedure InitLikeQuery(aDataset: TDBDataset);
  public
    constructor Create;
    destructor Destroy; override;
    procedure Assign(Source: TPersistent); override;
    ...
    procedure InitLikes(aDataset: TDBDataset);
    ...
    property LikeQuery: TQuery read FLikeQuery write FLikeQuery;
...
{ TCalcLink }
  ...
  TCalcLinkOption = (coResult, coVisible, coAggregate);
...
Uses TypInfo, Windows, SysUtils, BDE, BDEConst, Dialogs, FindDlgc, EtvDBFun, EtvOther,
     EtvPas, VgUtils, XApps, XDBTFC, XForms;
    ...
    QExtracts:=TDataSet(
      TDBFormControl(TXForm(Screen.ActiveForm).FormControl).
        ActivateMainQuery(nil, CreateSQLText(DataSet,GetTableName,'EFilter',
          ...
          SortingFromDataSet(DataSet),true,TableNameKind),false));
...
procedure TFindLink.InitLikeQuery(aDataset: TDBDataset);
begin
  if not Assigned(FLikeList) then FLikeList:=TList.Create;
  if not Assigned(FLikeQuery) then begin
    FLikeQuery:=TQuery.Create(aDataset.Owner);
    FLikeQuery.DatabaseName:=aDataset.DatabaseName;
    if not Assigned(FLikePatterns) then FLikePatterns:=TStringList.Create;
  end;
end;

procedure TFindLink.InitLikes(aDataset: TDBDataset);
begin
  if not Assigned(FLikeQuery) then begin
...
begin
  ...
  try
    ...
    if (FFindDataset is TLnQuery) and (TLnQuery(FFindDataset).CloneMaster=aDataset) then begin
      if aIFNItem.Fields<>IFNItem.Fields then SortingToDataSet(FFindDataSet,IFNItem.Fields,false)
    end else begin
...
begin
  ...
  if aOption=coAggregate then begin
...
Function TCalcLink.GetCalcFieldNames(aOption: TCalcLinkOption; aCalcName: String;
                   aAllFields: Boolean; aSepChar: Char;
                   AddNames: String): String;
var aList: TList;
    i: Integer;
  ...
  if aAllFields then begin
    ...
    for i:=0 to Fields.Count-1 do begin
      ...
      if aField.FieldKind=fkData then begin
        ...
        else S:= S+aField.FieldName+aSepChar;
      end;
    end;
  end else begin

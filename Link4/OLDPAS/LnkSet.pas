...
procedure ChangeLookQueryField(AField: TField; aLC:TDBLookupControlBorland; aLDataSet:TDataSet; aLookUpResultField:string; const TableName: TFileName);

Implementation

Uses Windows, DBITypes, DBIProcs, StdCtrls, Dialogs, Forms, XMisc,
     XApps, TypInfo, EtvDB, EtvLook, EtvDBFun, XForms, XDBTFC, EtvOther,
{$IFDEF UseXPrinter}
     XPrints, Progress,
{$ENDIF}
  ...
  end;
{
  with AQuery do begin
    ...
    for i:=0 to FieldCount-1 do begin
      if Fields[i].Lookup or Fields[i].Calculated then Continue;
      if Length(S)>57 then begin
        SQL.Add(SPref+S);
        SPref:='       ';
        S:='';
      end;
    ...
    if S<>'' then  begin
      ...
      S:= SPref+S+' from '+TableName;
      SQL.Add(S);
    end else begin
      Sql.Add(SPref+'* from '+TableName);
    end;
    S:=AInds;
    if S<>'' then begin
...
procedure ChangeLookQueryField(AField: TField; aLC:TDBLookupControlBorland;
          aLDataSet:TDataSet; aLookUpResultField:string; const TableName: TFileName);
...
begin
  ...
  and (aLDataSet is TLnQuery) then begin
    AQuery:=TLnQuery(aLDataSet);
    ...
    if i>0 then begin
      ...
      Repeat
        ...
        i:=Pos(';',S);
        S1:=Copy(S,1,i-1);
        if not(AQuery.FieldByName(S1).Lookup or AQuery.FieldByName(S1).Calculated) then
          Break;
        Dec(n);
      until n=0;

      for i:=0 to AQuery.FieldCount-1 do begin
        if AQuery.Fields[i].Calculated then Continue;
        if AQuery.Fields[i].Lookup then
          if AQuery.Fields[i].FieldName=S1 then begin
            AQuery.FieldByName(AQuery.Fields[i].KeyFields).Index:=AQuery.FieldCount-1;
            Break;
          end else Continue;
        AQuery.Fields[i].Index:=AQuery.FieldCount-1;
        Break;
      end;
      AQuery.SQL.Clear;
      ChangeLinkQuery(AQuery, TableName, S1);
...
*)
            SortingToDataSet(aDataSet,aIFN,false);
...
begin
  ...
  with TLnQuery(Result.DataSet) do begin
    ...
    if not Assigned(aQuery) then begin
      ...
      aFieldNames:=AllFieldNames(Result.DataSet,false,false);
...
Procedure TLinkSource.CreateLikeQuery;
begin
  if IsDeclar then LinkSets.DeclarLink.Find.InitLikes(Declar);
end;
...
begin
  if IsDeclar then
    Result:= LinkSets.DeclarLink.Find.ChangeLikeQuery(aDataset, TableName, aFieldsName, aStr)
  else Result:= False;
end;
{
...
begin
  if IsDeclar then LinkSets.DeclarLink.Find.LikeQuery:= aQuery;
end;
...
begin
  if TableName<>'' then
    LnkSet.ChangeLookQueryField(AField, aLC, aLDataSet, aLookUpResultField, TableName)
end;
...
begin
  ...
  try
    if dfAutoPost in FOptions then begin
      ...
      if MultiModified and Assigned(Lookup) and (Lookup is TLinkQuery) and (Lookup.Active) then begin
        MultiModified:= False;

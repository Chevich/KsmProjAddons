...
Implementation
Uses DsgnIntf, EtvTable, EtvDBFun, PopupTr;
...
begin
  try
    ...
    Tree.FDataLink.DataSet.Locate(TvgCustomDBTreeView(TreeView).DataFieldID,FBookmark,[]);
...
begin
  ...
  if Result and ((Node=nil) or (not Node.Expanded)) then begin
    ...
    try
      ...
      for i:= 0 to Node.Count-1 do UpdateNode(Node.Item[i]);
    finally
...
begin
  ...
  if Result then begin
    try
      if assigned(FDataLink.DataSet) then FDataLink.DataSet.CheckBrowseMode;
    except
      SysUtils.ShowException(ExceptObject, ExceptAddr);
    end;
    if FDataLink.DataSet.State in [dsInsert,dsEdit] then Result:=false;
  end;
end;
...
begin
  ...
  with DataSource.DataSet do begin
    ...
    EnableDataSetChanged;
    DisableControls;
    Mark:=FieldByName(DataFieldID).AsInteger;
    CreateBranches(nil, FRootID);
    case SortType of
      stText,stBoth : Self.AlphaSort;
      stData        : Self.CustomSort(DefaultSortProc,0);
    end;
    {Locate(DataFieldID+';'+DataFieldParentID,VarArrayOf([Mark,9]),[loPartialKey]);}
    GoToNode(Mark);
    {GoToBookMark(Mark);}
    {FreeBookMark(Mark);}
    EnableControls;
...
begin
  ...
  TvgDBTreeNode(Result).Bookmark :=FDataLink.DataSet.FieldByName(DataFieldID).AsInteger;

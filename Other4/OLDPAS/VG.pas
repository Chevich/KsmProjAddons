...
Uses Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs,
     CommCtrl, ComCtrls, DB, DBTables, DBCtrls, Nav;
  ...
  if Assigned(FOnSetRange) then FOnSetRange(Self, DataSet, ParentID)
    ...
    with DataSet as TTable do begin
      if AnsiCompareText(IndexFieldNames, DataFieldParentID) <> 0 then
        try
          SortingToDataSet(DataSet, DataFieldParentID, false);
        except
...
begin
  if fSyncDataSet then begin
    ...
    case Button of
      ...
      nbRW    : PostMessage(Handle,WM_KeyDown,VK_PRIOR,0);
      ...
      nbFF    : PostMessage(Handle,WM_KeyDown,VK_NEXT,0);

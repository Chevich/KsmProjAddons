...
begin
  ...
  if aRecordCount>MaxRecordExport then begin
    ShowMessage('Превышен предел количества строк '+IntToStr(MaxRecordExport)+#13+
                'Сообщите о пробелеме авторам');
    Exit;
  end;
{
  try
    ExV:=GetActiveOleObject('Excel.Application');  // Попытка найти запущенный Эксел
    // при отладке из среды Дэльфи - если Эксел не запущен, то здесь Дэльфи ругается - это нормально
    //OK := Trd_SetLocale;
  except
    try
...

{
  ...
  if ReportName<>'' then Sh.Name:=ReportName
    ...
    Sh.Name := 'Tornado Report '+IntToStr(ReportNumber);  //  Это имя странички в Экселе
  end;
  Sh.Activate;
}
  ...
  for i:=0 to aQuery.FieldCount-1 do begin
    ...
    { 65 - A, 66 - B, 67 - C, ... }
    NameCell:=Chr(i+65)+'1';
    { Устанавливаем соответствующую ширину ячеек }
...
//  Форматировать ячейку пока не получилось :(
    case aQuery.Fields[i].DataType of
      ...
      ftFloat     : WS.Range[NameCell,NameCell+MaxRecordExportStr].NumberFormat:='Основной';
    end;
{
    if aQuery.Fields[i] is TNumericField then
      WS.Columns.Range[NameCell,NameCell].NumberFormat:='Общий'
    else WS.Columns.Range[NameCell,NameCell].NumberFormat:='Текстовый'
}
  end;

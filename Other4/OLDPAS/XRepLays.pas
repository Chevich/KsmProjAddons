...
Uses SysUtils, ppClasUt, ppUtils, ppBands, ppDBBDE, ppMemo, EtvDB, EtvppCtl,
...
{  Result.Font.Style := Result.Font.Style + [fsBold];}
  ...
  Result.spHeight:=lWrappedText.Count*
   (aHeight+(Byte(PrintLink.TypePrint<>PdGraph)+1)*4);
(*
  MaxWidth:=0;
...
begin
  ...
  if PrintLink.DataCalc.TypeResult=rsLocal then begin
    ...
    if PrintLink.TypePrint<>PdGraph then lCalc.spTop:=lCalc.spTop+19;
  end;
  ...
  (poHorzLines in PrintLink.DataOptions) then begin
    ...
    if PrintLink.DataCalc.TypeResult in [rsLocal, rsGlobal] then begin
      if PrintLink.DataCalc.TypeResult=rsLocal then begin
        ...
        aBand:=FSummaryBand;
        if PrintLink.TypePrint<>PdGraph then DeltaTop:=35;
      end;
  ...
  if poHeaderBorder in PrintLink.DataOptions then begin
    ...
    if PrintLink.DataCalc.TypeResult in [rsLocal, rsGlobal] then begin
      if PrintLink.DataCalc.TypeResult=rsLocal then begin
        ...
        aBand:=FSummaryBand;
        if PrintLink.TypePrint<>PdGraph then DeltaTop:=35{17};
      end;
      {result right line }
...
{Lev}
    if {FPrintLink.TextLink.SkipTextLines}false and (lHeight mod 26>0) then
      lHeight:=(lHeight div 26)*26;
{Lev}
...
{Lev}
      if PrintLink.DataCalc.TypeResult=rsLocal then begin
        ...
        aBand:=FSummaryBand;
        if PrintLink.TypePrint<>PdGraph then DeltaTop:=11{17};
      end;
...
{Lev}
(*
    if FPrintLink.TextLink.SkipTextLines and (lHeight mod 26>0) then
      lHeight:=(lHeight div 26)*26;
*)
{Lev}
...
{Lev}
      if PrintLink.DataCalc.TypeResult=rsLocal then begin
        ...
        aBand:=FSummaryBand;
        if PrintLink.TypePrint<>PdGraph then DeltaTop:=11;{17}{11}
      end;
...
{Lev}
      if FPrintLink.TypePrint<>pdGraph then begin
        ...
        if lBand is TppTitleBand then lBand.spHeight:=(lBand.spHeight div 26)*26+26;
(*

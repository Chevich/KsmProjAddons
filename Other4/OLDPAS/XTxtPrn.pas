...
{ TXTextPrinter }
  ...
  public
    ...
    procedure PrintAnsiStr(Str:string; InitOrDownLine:boolean);
...
{Lev!!!} { По-моему здесь можно ускорить, если присвоить Result:=СТРОКА ИЗ ПРОБЕЛОВ,}
...
begin
  ...
  end;                    { WIN95 }            {'Размеры, определяемые пользователем'}
...
begin
  ...
  FPageNumber:=0;
  FStartPage:=False;
  if PrinterHandle=0 then raise EInOutError.Create('Ошибка при подключении к принтеру');
  ...
  if not WinSpool.GetPrinter(FPrinterHandle,2,FPrinterInfo2,FSizePrinterInfo2,
    @FSizePrinterInfo2) then raise EInOutError.Create('Ошибка при подключении к принтеру');
  ...
  if WinSpool.StartDocPrinter(FPrinterHandle,1,@ADocInfo1)=0 then
    raise EInOutError.Create('Ошибка при открытии документа для принтера');
  GetIC;
  StartPage;
end;
...
begin
  ...
  if not FAborted then
    if not EndDocPrinter(FPrinterHandle) then
      raise EInOutError.Create('Ошибка при закрытии документа');
  FreePrinterHandle;
  FreeIC;
  FPrinting:=False;
  FStartPage:=False;
...
{ Lev 21.12.98 параметр BegI устанавливается в 1 если InitLine=''   }
{ что позволяет избежать вывода 1-й пустой строки при печати бланка }
...
begin
  ...
  { При i=0 печатаем FInitLine }
  for i:=BegI{0} to Canvas.Lines.Count-1 do PrintAnsiStr(Canvas.Lines[i], false);
  {if Canvas.Lines[Canvas.Lines.Count-1]}
  { При i=Count-1 печатаем FDoneLine }
  if FDoneLine<>'' then PrintAnsiStr(FDoneLine+#13#10,true);
  Str:=#12;
  if not WritePrinter(FPrinterHandle, PChar(Str), Length(Str), PLen) then
    raise EInOutError.Create('Ошибка конца страницы');
end;
...
{!?    Ultimate;}
    raise EInOutError.Create('Ошибка при печати на принтер');
  end;
end;

Procedure TXTextPrinter.PrintAnsiStr(Str:string; InitOrDownLine:boolean);
var StrT:string; plen:dword;
  ...
  begin
    if not InitOrDownLine then begin
      if Length(Str)>255 then CharToOEMBuff(PChar(Str),FBufferStr,255)
      else CharToOEM(PChar(Str),FBufferStr);

...
Uses TypInfo, SysUtils, Controls, DiDual,
     ...
     LnkSet, XForms, XExps, DLinksEd, XReports;
...
Procedure SwapSources(AComponent: TDBFormControl; ADesigner: TFormDesigner);
type
...
begin
  ...
  if Assigned(Comp.Form) then
    Result := TFormDesigner(Comp.Form.Designer).GetComponentName(TComponent(GetOrdValue))
  else Result:='';
end;
...
begin
  Comp:=TFormControl(GetComponent(0));
  if Assigned(Comp.Form) then
    TFormDesigner(Comp.Form.Designer).GetComponentNames(GetTypeData(GetPropType), Proc);
end;
...
begin
  if Value = '' then Component := nil
    ...
    Comp:=TFormControl(GetComponent(0));
    if Assigned(Comp.Form) then
      Component := TFormDesigner(Comp.Form.Designer).GetComponent(Value)
    else Component:=nil;

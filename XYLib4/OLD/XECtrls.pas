...
Uses Windows, Dialogs, Forms, LnTables, EtvBor, LnkSet, EtvDB, FVisDisp,EtvPopup,
...
Procedure TXEDBEdit.Notification(AComponent: TComponent;
...
begin
  ...
  if (FIsContextOpen<>AValue) and Assigned(FStoreDataSet) then
    ...
    if AValue then begin
      ...
      Self.HeadColor:=clYellow;
      Self.HeadLineStr:='Выборка по "'+
        TLinkQuery(FStoreDataSet).LinkSource.LikePatterns[0]+'" - '+
          IntToStr(TLinkQuery(FStoreDataSet).LinkSource.LikeQuery.RecordCount)+' записей';
...
begin
  ...
  (TDBLookupControlBorland(Self).FKeyFieldName<>'')) then
    XEDBContextInit(Self, FIsContextOpen, FStoreColor, FStoreHeadColor, FStoreDataSet);

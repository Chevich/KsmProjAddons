  ...
  begin
    ...
    try
      ...
      ISourceStream := TIStreamAdapter.Create(TStringStream.Create(
        Format(GetStrSource, [UnitIdent,FormIdent])),True);
      try
...
Procedure TXDBFormDesignModule.ValidateComponent(Component: TComponent);
...
begin
  ...
  if TXForm(Root).FormControl is TDBFormControl then begin
    ...
    if Assigned(ALink) then begin
      ...
      if PropInfo<>nil then begin
        SetOrdProp(Component, PropInfo, LongInt(ALink));
      end;
    end;
  end;
end;

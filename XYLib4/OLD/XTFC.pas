...
Uses SysUtils, DBCtrls, DBIndex, XAbout;
  ...
  if Assigned(FSplashForm) then
    with TXSplashForm(FSplashForm) do begin
      if FInformActive then begin
        XHardLab:=FInform.OrgName;
        XTaskLab:=FInform.TaskName;
        XAutsLab:=FInform.AuthorsName;
...
begin
  ...
  AboutBox.ComputerName.Caption :=Buffer;
...
begin
  ...
  else if Assigned(SystemControlMenu) then SystemControlMenu.Popup(X, Y);
end;
...
begin
  if FActive then begin
    ...
    if Screen.ActiveForm<>Form then

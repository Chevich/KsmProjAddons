...
Uses TypInfo, Windows, SysUtils, Messages, Dialogs, Buttons, XDBForms, ComCtrls,
     ...
     EtvTable, EtvForms, BEForms, EtvDBFun, EtvTemp, TlsForm, ExpExcel, EtvOther
{$IFDEF Report_Builder}
     , XTxtDev, ppComm
{$ENDIF}
     ;
...
begin
  ...
  if (FCurrentSource is TLinkSource) and Assigned(FCurrentSource.DataSet) then begin
    ...
    if Assigned(aItem) then begin
      ...
      if not Assigned(FCurrentInquiryItem) then OldIFNItem:=aLink.IFNItem;
      ...
      { Внутри ActivateFilter происходит FCurrentSource.DataSet.Open;}
  ...
  case TComponent(Sender).Tag of
    ...
    btSum:
      if Assigned(Form) and (Sender is TSpeedButton) and (FCurrentSource is TLinkSource) then begin
        ...
        if XNotifyEvent.SpellEvent<>xeNone then ShowMessage('Вычисления!');
      ...
      begin
        if (Self.Form.ActiveControl=TBEForm(Self.Form).Grid) then begin
          if Assigned(TBEForm(Self.Form).Grid.DataSource) then begin
            TBEForm(Self.Form).Grid.FormatColumns;
            TBEForm(Self.Form).Grid.DataSource:=nil;
            TBEForm(Self.Form).Grid.DataSource:=CurrentSource;
          end
        end else
          SetMinLengthFieldsByDataSet(TLinkTable(TLinkSource(CurrentItem.Source).DataSet));
      end;
    btExportToExcel:
      if (AppUserName='LEV') or (AppUserName='IGO') or (AppUserName='BAUSHA') then begin
        aDataSet:=GetSet;
        try
          ExportToExcel(TQuery(aDataSet));
        finally
          aDataSet.Free;
        end;
      end else
        ShowMessage('Для работы данной функции необходимо написать служебную записку'+#13+
          'на имя главного экономиста');
{Lev }
...
Procedure LinkFilterSave(EtvFilter:TEtvFilter);
...
procedure LinkFilterLoad(EtvFilter:TEtvFilter);
var Stream:TMemoryStream;
    Size: integer;
    aLink: TAggregateLink;
begin
  with TEtvFilterSelf(EtvFilter) do begin
    if Assigned(DataSet) and Assigned(fOrigDataSet) then begin
      try
        Stream:=nil;
        (*
        if DataSet is TLnTable then Stream:=TLnTable(DataSet).FilterStream

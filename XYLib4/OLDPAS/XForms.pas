...
Uses Classes, Controls, Forms, Messages, XMisc;
...
{  if Assigned(XFormLink) then }
  ...
  if Assigned(AControl) then
    with TFormControl(AControl) do begin
      if Priz then with FormRect do if Active then begin
        Left:=FormLeft;
        Top:=FormTop;
        Width:=FormWidth;
        Height:=FormHeight;
...
{  if Assigned(XFormLink) and Assigned(XFormLink.LinkControl) then
  ...
  if Assigned(FormControl) and not Application.Terminated then begin

...
// ask the user whether the current process may execute the specified command line
{
  ...
  if (!AmSystemProcess()) {
    ...
    // let's allow system processes do execute whatever they want
    return true;
}
                                   ...
                                   LPPROCESS_INFORMATION lpProcessInformation)
{
  if (!IsAllowed(lpApplicationName, lpCommandLine, NULL, NULL)) {
    ...
    DWORD result = CreateProcessANext(lpApplicationName, lpCommandLine,
                                   ...
                                   LPPROCESS_INFORMATION lpProcessInformation)
{
  if (!IsAllowed(NULL, NULL, lpApplicationName, lpCommandLine)) {
    ...
    DWORD result = CreateProcessWNext(lpApplicationName, lpCommandLine,
                                      lpProcessAttributes, lpThreadAttributes,
                                      bInheritHandles, dwCreationFlags,
                                      lpEnvironment, lpCurrentDirectory,
                                      lpStartupInfo, lpProcessInformation);

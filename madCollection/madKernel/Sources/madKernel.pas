// ***************************************************************
//  madKernel.pas             version:  1.3k  ·  date: 2009-02-09
//  -------------------------------------------------------------
...
constructor TIKernelObj.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: AnsiString;
...
begin
  ...
  if FHandle = nil then
    FHandle := TIHandle.Create(false, CErrorNo_AmInvalid, CErrorStr_AmInvalid, 0, 0, false, nil, nil, otUnknown);
  result := FHandle;
end;
...
var semaphore, event, mutex, process, thread, file_, section, token : dword;
    arrChW : array [0..MAX_PATH] of wideChar;
    i1     : integer;
    c1     : dword;
    buf    : TPAPointer;
...
constructor TIHandle.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: AnsiString;
                            processID, handle: cardinal; autoClose: boolean;
                            hi95: TP9xHandleItem; hiNt: TPNtHandleItem;
                            typ: TKernelObjType);
var c1  : cardinal;
...
constructor TIHandles.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: AnsiString;
...
begin
  if (index < 0) or (index >= FCount) then
       result := TIHandle.Create(false, CErrorNo_IndexOutOfRange, CErrorStr_IndexOutOfRange, 0, 0, false, nil, nil, otUnknown)
...
constructor TIWindow.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: AnsiString;
                            handle, tid, pid: cardinal);
begin
  inherited Create(valid, lastErrorNo, lastErrorStr);
  if FValid then begin
...
constructor TIWindows.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: AnsiString;
                             const thread: IThread; const process: IProcess; inTaskbar: boolean;
                             const objects: array of IBasic);
var i1 : integer;
begin
...
constructor TITrayIcon.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: AnsiString;
                              trayIcons: TITrayIcons; const nid: TNotifyIconDataA; imageIndex: integer; rect: PRect);
begin
  inherited Create(valid, lastErrorNo, lastErrorStr);
  if FValid then begin
...
constructor TITrayIcons.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: AnsiString;
                               const process: IProcess; const objects: array of IBasic);
var i1 : integer;
begin
  inherited Create(valid, lastErrorNo, lastErrorStr);
...
begin
  if (index < 0) or (index >= FCount) then
       result := TITrayIcon.Create(false, CErrorNo_IndexOutOfRange, CErrorStr_IndexOutOfRange, nil, nid, 0, nil)
...
constructor TIExportEntry.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: AnsiString;
                                 pid: cardinal; moduleName: AnsiString; moduleHandle: cardinal;
                                 ordinal: cardinal; name: AnsiString; address: pointer);
begin
  inherited Create(valid, lastErrorNo, lastErrorStr);
...
constructor TIModule.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: AnsiString;
                            const process: IProcess;
                            handle: cardinal; fileName_: AnsiString; autoClose: boolean);
var i1  : integer;
...
constructor TIModules.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: AnsiString;
                             const process: IProcess; fileName: AnsiString; const objects: array of IBasic);
var i1 : integer;
begin
  inherited Create(valid, lastErrorNo, lastErrorStr);
...
constructor TIThreads.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: AnsiString;
                             const process: IProcess; const objects: array of IBasic);
var i1 : integer;
begin
  inherited Create(valid, lastErrorNo, lastErrorStr);
...
begin
  ...
  if CreateProcessA(nil, PAnsiChar(s1), processAttr, threadAttr, false,
...
constructor TIProcesses.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: AnsiString;
                               exeFile: AnsiString; const objects: array of IBasic);
var i1 : integer;
begin
  inherited Create(valid, lastErrorNo, lastErrorStr);
...
constructor TINamedBuffer.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: AnsiString;
                                 name: AnsiString; size: integer; writeAccess: boolean;
                                 buffer: pointer; handle: cardinal);
begin
  if valid then

// ***************************************************************
//  madShell.pas              version:  1.3q  ·  date: 2009-02-09
//  -------------------------------------------------------------
...
constructor TIIDList.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: AnsiString;
                            pidl: PItemIDList; copy: boolean = false);
var c1 : cardinal;                            
begin
...
constructor TIShortCut.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: AnsiString;
                              const idList: IIDList; fileName: AnsiString);
var c1    : cardinal;
    pidl1 : PItemIDList;
begin
    ...
    if (FFile <> '') or (shortCutFileName <> '') then begin
      try
        if FDirty or ((shortCutFileName <> '') and (not IsTextEqual(shortCutFileName, FFile))) then begin
          ...
          s1 := AnsiString(ExtractFileExt(string(shortCutFileName)));
...
constructor TIShellObj.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: AnsiString;
                              const parentShellFolder: IShellFolder;
                              const idList, childIdList: IIDList);
var c1 : cardinal;
begin
            ...
            if p1 <> nil then
              try
                ...
                if WriteProcessMemory(ph, p1, @buf, sizeOf(buf), c1) and (c1 = sizeOf(buf)) then begin
                  i1 := SendMessageA(lv, LVM_FINDITEMA, -1, integer(@p1^.findItem));
                  if i1 <> -1 then begin
                    ...
                    try
                      ...
                      for i1 := 0 to high(desktopItems) do begin
                        ...
                        if WriteProcessMemory(ph, p1, @buf, sizeOf(buf), c1) and (c1 = sizeOf(buf)) and
                           (SendMessageA(lv, LVM_GETITEMPOSITION, i1, integer(@p1^.pos)) <> 0) and
                           (SendMessageA(lv, LVM_GETITEM, 0, integer(@p1^.lvItem)) <> 0) and
  ...
  if shgdn = SHGDN_FORPARSING then begin
      ...
      repeat
        if (sf1 <> nil) and (sf1.QueryInterface(IPersistFolder, pf) = 0) and
          ...
          case sr.uType of
            ...
            STRRET_OFFSET : str := PAnsiChar(cardinal(idl2.pidl) + sr.uOffset) + '\' + str;
    ...
    if result then begin
      case sr.uType of
        ...
        STRRET_OFFSET : str := PAnsiChar(cardinal(FChildIDList.pidl) + sr.uOffset);
...
begin
  ...
  infoRec.old       := old;
  infoRec.new       := new_;
  SendMessageA(MsgHandlerWindow(FThreadID), FWatchMsg, 789, integer(@infoRec));
end;
...
constructor TIDisplayMode.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: AnsiString;
                                 const devMode: TDevMode);
begin
  inherited Create(valid, lastErrorNo, lastErrorStr);
  if FValid then
...
constructor TIDisplayModes.Create(valid: boolean; lastErrorNo: cardinal; lastErrorStr: AnsiString);
